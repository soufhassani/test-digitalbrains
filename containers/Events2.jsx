...
const Events = () => {
  ...
  const conditionLeft = (index) => {
    if (WindowSize <= 450) return `${(index - position) * 55 + 20}vw`;
    if (WindowSize > 450) return `${(index - position) * 225 + 92}px`;
  };
      ...
      <div className=" min-h-[250px] h-[250px] relative max-w-[400px] mx-auto md:h-[300px] ">
        ...
        >
          {cards.map((card, index) => (
            <m.div
              className={`absolute h-[250px]  w-[55vw] ${
                WindowSize >= 450 && "w-[231px]"
        ...
        <div
          className={`group z-40 w-[87px] h-full flex justify-start items-center absolute top-0 left-[-10px] scale-[0.8] pl-1 bg-white/40 rounded-[5px] ${
            position === 0 && "hidden"
          }`}
        ...
        <div
          className={`group z-40 w-[87px] h-full flex justify-end items-center absolute top-0  right-[-10px] pr-1 scale-[0.8] bg-white/40  rounded-[5px] ${
